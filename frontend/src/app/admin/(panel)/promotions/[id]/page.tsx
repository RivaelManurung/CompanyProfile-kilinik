...
function toFormValues(data?: Promotion): FormValues {
  return {
    title: data?.title ?? "",
    tag: data?.tag ?? "",
    price: data?.price ?? "",
    ...
    desc: data?.desc ?? "",
    slug: data?.slug ?? "",
    status: data?.status || (data?.active ? "active" : "draft"),
    campaignType: data?.campaignType ?? "discount",
    ...
    targetAudience: (data?.targetAudience as FormValues["targetAudience"]) ?? "general",
  };
}
...
export default function EditPromotionPage() {
  ...
  useEffect(() => {
    if (!slugManuallyEdited.current && watchedTitle && promotion && watchedTitle !== promotion.title) {
      form.setValue("slug", generateSlug(watchedTitle), { shouldValidate: false });
    }
  }, [watchedTitle, form, promotion]);

  const discountPercent = useMemo(() => {
    const p = parsePrice(watchedPrice);
    const op = parsePrice(watchedOldPrice);
    if (op > 0 && p > 0 && op > p) {
      return Math.round((1 - p / op) * 100);
    }
    return 0;
  }, [watchedPrice, watchedOldPrice]);

  const priceValid = Boolean(watchedPrice) && Boolean(watchedOldPrice) && parsePrice(watchedPrice) > 0 && parsePrice(watchedOldPrice) > parsePrice(watchedPrice);
  ...
  function renderField(
    ...
    const { type = "text", placeholder, hint, required, full } = options || {};
    ...
    return (
      <FieldGroup key={name} className={full ? "md:col-span-2" : undefined}>
        ...
        {type === "textarea" ? (
          <Textarea id={name} rows={4} placeholder={placeholder} {...form.register(name)} aria-invalid={Boolean(errorMessage)} />
        ) : type === "switch" ? (
          ...
          <Input
            id={name}
            type={type === "number" ? ("number" as const) : "text"}
          ...
          <div className="space-y-6 xl:col-span-2">
            <FormShell title="Identitas Kampanye" description="Informasi dasar promosi.">
              <FormGrid>
                ...
                {renderField("tag", "Tag / Badge", { placeholder: "Hemat 20%", hint: "Label pendek di badge (maks 20 karakter)" })}
