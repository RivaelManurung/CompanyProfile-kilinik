...
function toForm(article?: Article): FormValues {
  return {
    title: article?.title ?? "",
    slug: article?.slug ?? "",
...
export default function EditArticlePage() {
  ...
  const isIdValid = id !== null && !isNaN(id);
  ...
  async function save(publishStatus: "draft" | "published" | "scheduled" | "archived") {
    ...
    if (!valid || !id) {
      if (!valid) {
        ...
        toast.error(
          (first?.message as string) || "Mohon periksa kembali isian form",
        );
      }
      return;
    }
